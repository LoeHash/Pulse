package com.ruoyi.system.recommend.recall.impl;
...
import static com.ruoyi.system.recommend.chain.impl.DefaultRecallChain.EXPOSED_FILTER_DAYS;
...
public class HotRecallStrategy implements RecallStrategy,
        DynamicConfigListener<AlgoConfig>{

    /** like 权重 */
    private static double LIKE_FACTOR = 2D;
    /** comment 权重 */
    private static double COMMENT_FACTOR = 3D;
    /** favorite 权重 */
    private static double FAVORITE_FACTOR = 2D;
    /** view 权重 */
    private static double VIEW_FACTOR = 0.1D;
    /** 时间衰减系数（小时粒度） */
    private static double TIME_DECAY = 0.03D;


    private final ForumPostMapper postMapper;
    private final com.ruoyi.system.recommend.service.IRecommendExposureService exposureService;
    /**
     * 热门召回
     *
    ...
    public void onChange(AlgoConfig newConfig) {
        synchronized (this){
            LIKE_FACTOR = newConfig.getLikeFactor();
            COMMENT_FACTOR = newConfig.getCommentFactor();
            FAVORITE_FACTOR = newConfig.getFavoriteFactor();
            VIEW_FACTOR = newConfig.getViewFactor();
            TIME_DECAY = newConfig.getTimeDecay();
        }
    }
}