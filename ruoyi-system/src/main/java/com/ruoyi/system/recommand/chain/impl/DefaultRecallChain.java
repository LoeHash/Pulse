package com.ruoyi.system.recommand.chain.impl;
...
public class DefaultRecallChain implements RecallChain {
     ...
     * 各召回路在融合时的权重
     */
    private static final Map<String, Double> CHANNEL_WEIGHT = Map.of(
            "latest", 0.20D,
            "hot", 0.05D,
            "preference", 0.75D
    );
     ...
     */
    private static final double ROAD_HIT_FACTOR = 0.05D;

    /**
     * 新增：曝光去重窗口（天）
     */
    public static final int EXPOSED_FILTER_DAYS = 7;

    /**
     * 新增：允许复推比例（当新内容不足时，最多补多少“已曝光内容”）
     */
    private static final double REPEAT_RATIO = 0.30D;
